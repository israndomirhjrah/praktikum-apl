...
void menuAdmin(User* currentUser, Tim* daftarTim, int* jumlahTim, int* nextIdTim) {
    ...
    do {
        ...
        cout << "6. melihat tim\n";
        cout << "7. melihat poin tim\n";
        cout << "8. tahun berdiri tim\n";
        cout << "9. Logout\n\n";

        pilihan = inputAngka("Pilih menu (1-9): ", 1, 9);
        ...
        switch (pilihan) {
            ...
            case 6:
                for (int i = 0; i < *jumlahTim; i++) timTemp[i] = daftarTim[i];
                NamaAscending(timTemp, *jumlahTim);
                tampilkanTim(timTemp, *jumlahTim, "URUTAN NAMA (A-Z)");
                break;
            case 7:
                for (int i = 0; i < *jumlahTim; i++) timTemp[i] = daftarTim[i];
                PoinDescending(timTemp, *jumlahTim);
                tampilkanTim(timTemp, *jumlahTim, "URUTAN POIN (TERTINGGI -> TERENDAH)");
                break;
            case 8:
                for (int i = 0; i < *jumlahTim; i++) timTemp[i] = daftarTim[i];
                TahunAscending(timTemp, *jumlahTim);
                tampilkanTim(timTemp, *jumlahTim, "URUTAN TAHUN BERDIRI (TERTUA -> TERMUDA)");
                break;
            case 9:
                cout << "\nLogout berhasil!\n";
                running = false;
...
void menuUser(User* currentUser, Tim* daftarTim, int* jumlahTim, int* nextIdTim) {
    ...
    do {
        ...
        cout << "3. melihat tim\n";
        cout << "4. melihat poin Tim \n";
        cout << "5. tahun berdiri Tim \n";
        cout << "6. Logout\n\n";
